...
Status VerbsServer::Init(ServiceInitFunction service_func,
  ...
  {
    mutex_lock l(mu_);
    adapter = new RdmaAdapter(worker_env());
  }
  std::call_once(reg_mem_visitors_call, [adapter]() {
    adapter->RegMemVisitors();
  });
  Status s = GrpcServer::Init(service_func, rendezvous_mgr_func);
  {
    ...
    rdma_mgr_ = new RdmaMgr(worker_env(), channel_cache_, adapter);
