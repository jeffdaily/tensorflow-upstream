...
def set_tf_rccl_path(environ_cp):
  """Set TF_RCCL_INSTALL_PATH."""
  if not is_linux():
    raise ValueError('Currently RCCL is only supported on Linux platform.')

  key = 'TF_RCCL_INSTALL_PATH'
  if key not in environ_cp:
    ask_rccl_path = (
        'Please specify the path to the locally installed RCCL you want to use. '
        '[Leave empty to use https://github.com/ROCmSoftwarePlatform/rccl]: ')
    tf_rccl_path = get_from_env_or_user_or_default(environ_cp, key,
                                                   ask_rccl_path, '')
    environ_cp[key] = tf_rccl_path
  write_action_env_to_bazelrc(key, environ_cp[key])
